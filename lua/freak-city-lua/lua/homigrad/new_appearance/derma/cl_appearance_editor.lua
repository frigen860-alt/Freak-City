...
colors.mainText = Color(0,0,0)
colors.secondaryText = Color(0,0,0,125)
...
colors.scrollbarGripHover = Color(0,0,0)
colors.scrollbarBorder = Color(100,100,120,200)
colors.previewBorder = Color(255,200,50,255)

local FC_BG = Material("freakcity/menu/bg2.png")
local FC_BUTTON = Material("freakcity/menu/button.png", "smooth")

surface.CreateFont("FreakCity", {
...
local function PaintPaperButton(self, w, h)
    if not FC_BUTTON then return end

  surface.SetMaterial(FC_BUTTON)
...
function PANEL:PostInit()
    ...
    self.AppearanceTable = istable(self.AppearanceTable) and self.AppearanceTable or {}
    self.AppearanceTable.AName = tostring(self.AppearanceTable.AName or "")
    self.AppearanceTable.AAttachments = istable(self.AppearanceTable.AAttachments) and self.AppearanceTable.AAttachments or {"none", "none", "none"}
    self.AppearanceTable.AClothes = istable(self.AppearanceTable.AClothes) and self.AppearanceTable.AClothes or {}
    self.AppearanceTable.ABodygroups = istable(self.AppearanceTable.ABodygroups) and self.AppearanceTable.ABodygroups or {}
    self.AppearanceTable.AColor = self.AppearanceTable.AColor or Color(255, 255, 255)

    local playerModels = APmodule.PlayerModels or {}
    local maleModels = playerModels[1] or {}
    local femaleModels = playerModels[2] or {}
    local modelName = self.AppearanceTable and self.AppearanceTable.AModel
    local tMdl = maleModels[modelName] or femaleModels[modelName]

    if not tMdl then
        local firstName, firstData = next(maleModels)
        if not firstData then firstName, firstData = next(femaleModels) end
        if firstData then
            tMdl = firstData
            self.AppearanceTable.AModel = firstName
        else
            tMdl = {
                mdl = "models/player/group01/female_01.mdl",
                submatSlots = {},
                sex = true
            }
            self.AppearanceTable.AModel = "Fallback"
        end
    end

    tMdl.submatSlots = tMdl.submatSlots or {}
    ...
    function viewer:LayoutEntity(Entity)
        ...
        tbl.AAttachments = istable(tbl.AAttachments) and tbl.AAttachments or {"none", "none", "none"}
        tbl.AClothes = istable(tbl.AClothes) and tbl.AClothes or {}
        tbl.ABodygroups = istable(tbl.ABodygroups) and tbl.ABodygroups or {}
        tbl.AColor = tbl.AColor or Color(255, 255, 255)
        local pm = APmodule.PlayerModels or {}
        local pm1 = pm[1] or {}
        local pm2 = pm[2] or {}
        tMdl = pm1[tbl.AModel] or pm2[tbl.AModel] or tMdl
        if not tMdl then
            tMdl = { mdl = "models/player/group01/female_01.mdl", submatSlots = {}, sex = true }
        end
        tMdl.submatSlots = tMdl.submatSlots or {}
        tbl.AColor = tbl.AColor or Color(255,255,255)
    ...
    modelSelector:SetFont("FreakCity")
    modelSelector:SetText(tostring(main.AppearanceTable.AModel or ""))
    ...
    for k, v in SortedPairs((APmodule.PlayerModels and APmodule.PlayerModels[1]) or {}) do
        modelSelector:AddChoice(k)
    end
    for k, v in SortedPairs((APmodule.PlayerModels and APmodule.PlayerModels[2]) or {}) do
        modelSelector:AddChoice(k)
    end
    ...
    backViewButton:SetFont("FreakCity")
    backViewButton:SetText("Rotate")
    backViewButton:SetTextColor(Color(0,0,0))
    ...
    ApplyButton:SetFont("FreakCity")
    ApplyButton:SetText("Apply")
    ApplyButton:SetTextColor(Color(0,0,0))
    ...
    NameEntry:SetFont("FreakCity")
    NameEntry:SetText(tostring(main.AppearanceTable.AName or ""))
    ...
    -- Панель с пресетами (Save/Load/Delete)
    ...
    savePresetBtn:SetFont("FreakCity")
    savePresetBtn:SetText("Save")
    savePresetBtn:SetTextColor(Color(0,0,0))
    savePresetBtn:DockMargin(0,0,5,0)
    function savePresetBtn:Paint(w, h)
        PaintPaperButton(self,w,h)
        local bgCol = Color(255,255,255,0)
    ...
    loadPresetBtn:SetFont("FreakCity")
        ...
        for _, presetName in SortedPairs(presetList) do
            ...
            function presetBtn:DoClick()
                ...
                if loadedPreset then
                    ...
                    NameEntry:SetText(tostring(loadedPreset.AName or ""))
                    modelSelector:SetText(tostring(loadedPreset.AModel or "Male 01"))
                    presetNameEntry:SetText(tostring(presetName or ""))
                    surface.PlaySound("buttons/button14.wav")
    ...
    deletePresetBtn:SetFont("FreakCity")
    ...
    presetNameEntry:SetFont("FreakCity")
    ...
    -- Hats (закруглённая кнопка)
    local hatSelector = vgui.Create("DButton", viewer)
    hatSelector:SetSize(ScreenScale(100), ScreenScale(18))
    hatSelector:SetFont("FreakCity")
    hatSelector:SetText("Hats")
    SetupCharacterButton(hatSelector)
    hatSelector:MoveToFront()
    function hatSelector:Think()
        if funpos1x then hatSelector:SetPos(leftButtonsX + funpos1x, buttonsTopY) end
    end
    ...
    -- Face (закруглённая кнопка)
    local faceSelector = vgui.Create("DButton", viewer)
    faceSelector:SetSize(ScreenScale(100), ScreenScale(18))
    faceSelector:SetFont("FreakCity")
    faceSelector:SetText("Face")
    SetupCharacterButton(faceSelector)
    faceSelector:MoveToFront()
    function faceSelector:Think()
        if funpos1x then faceSelector:SetPos(leftButtonsX + funpos1x, buttonsTopY + ScreenScale(32)) end
    end
    ...
    -- Body (закруглённая кнопка)
    local bodySelector = vgui.Create("DButton", viewer)
    bodySelector:SetSize(ScreenScale(100), ScreenScale(18))
    bodySelector:SetFont("FreakCity")
    bodySelector:SetText("Body")
    SetupCharacterButton(bodySelector)
    bodySelector:MoveToFront()
    function bodySelector:Think()
        if funpos3x then bodySelector:SetPos(leftButtonsX + funpos1x, buttonsTopY + ScreenScale(64)) end
    end
    ...
    -- Jacket (закруглённая кнопка)
    local bodyMatSelector = vgui.Create("DButton", viewer)
    bodyMatSelector:SetSize(ScreenScale(100), ScreenScale(18))
    bodyMatSelector:SetFont("FreakCity")
    bodyMatSelector:SetText("Jacket")
    SetupCharacterButton(bodyMatSelector)
    bodyMatSelector:MoveToFront()
    function bodyMatSelector:Think()
        if funpos3x then bodyMatSelector:SetPos(rightButtonsX - funpos3x, buttonsTopY) end
    end
    ...
    -- Pants (закруглённая кнопка)
    local legsMatSelector = vgui.Create("DButton", viewer)
    legsMatSelector:SetSize(ScreenScale(100), ScreenScale(18))
    legsMatSelector:SetFont("FreakCity")
    legsMatSelector:SetText("Pants")
    SetupCharacterButton(legsMatSelector)
    legsMatSelector:MoveToFront()
    function legsMatSelector:Think()
        if funpos3x then legsMatSelector:SetPos(rightButtonsX - funpos3x, buttonsTopY + ScreenScale(32)) end
    end
    ...
    function legsMatSelector:DoClick()
        ...
        for k, v in SortedPairs(hg.Appearance.Clothes[tMdl.sex and 2 or 1]) do
            legsMatSelectorMenu:AddOption(k, function()
                surface.PlaySound("player/weapon_draw_0"..math.random(2,5)..".wav")
    ...
    -- Boots (закруглённая кнопка)
    local bootsMatSelector = vgui.Create("DButton", viewer)
    bootsMatSelector:SetSize(ScreenScale(100), ScreenScale(18))
    bootsMatSelector:SetFont("FreakCity")
    bootsMatSelector:SetText("Boots")
    SetupCharacterButton(bootsMatSelector)
    bootsMatSelector:MoveToFront()
    function bootsMatSelector:Think()
        if funpos3x then bootsMatSelector:SetPos(rightButtonsX - funpos3x, buttonsTopY + ScreenScale(64)) end
    end
    ...
    function bootsMatSelector:DoClick()
        ...
        for k, v in SortedPairs(hg.Appearance.Clothes[tMdl.sex and 2 or 1]) do
            bootsMatSelectorMenu:AddOption(k, function()
                surface.PlaySound("player/weapon_draw_0"..math.random(2,5)..".wav")
    ...
    -- Gloves (закруглённая кнопка)
    local glovesSelector = vgui.Create("DButton", viewer)
    glovesSelector:SetSize(ScreenScale(100), ScreenScale(18))
    glovesSelector:SetFont("FreakCity")
    glovesSelector:SetText("Gloves")
    SetupCharacterButton(glovesSelector)
    glovesSelector:MoveToFront()
    function glovesSelector:Think()
        if funpos3x then glovesSelector:SetPos(rightButtonsX - funpos3x, buttonsTopY + ScreenScale(96)) end
    end
    ...
    -- Facemap (закруглённая кнопка)
    local faceMatSelector = vgui.Create("DButton", viewer)
    faceMatSelector:SetSize(ScreenScale(100), ScreenScale(18))
    faceMatSelector:SetFont("FreakCity")
    faceMatSelector:SetText("Facemap")
    SetupCharacterButton(faceMatSelector)
    faceMatSelector:MoveToFront()
    function faceMatSelector:Think()
        if funpos3x then faceMatSelector:SetPos(rightButtonsX - funpos3x, buttonsTopY + ScreenScale(128)) end
    end
...
concommand.Add("hg_appearance_menu", function() print('use esc menu') end)
