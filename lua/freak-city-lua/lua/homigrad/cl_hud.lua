...
-- FreakCity newspaper font for the Q radial selector.
surface.CreateFont("FC_Q_Newspaper", {
...
surface.CreateFont("FC_Q_Newspaper_Big", {
    font = "Special Elite (Rus by Lomzz)",
    size = ScreenScale(13),
    weight = 600,
    antialias = true,
...
function draw.CirclePart(x, y, radius, seg, parts, pos, scaleX, scaleY)
	scaleX = scaleX or 1
	scaleY = scaleY or 1
	...
	for i = 0, seg do
		...
		table.insert(cir, {
			x = x + math.sin(a) * radius * scaleX,
			y = y + math.cos(a) * radius * scaleY,
			u = math.sin(a) / 2 + 0.5,
			v = math.cos(a) / 2 + 0.5
		})
	end
...
-- One PNG is reused for every sector.
-- File location:
-- materials/freakcity/menu/q_sector.png
--
-- IMPORTANT: q_sector.png should have a transparent background.

local FC_Q_SECTOR = Material("freakcity/menu/q_sector.png", "smooth noclamp")

-- ============================================================
-- НАСТРОЙКА РАЗМЕРА PNG Q-СЕЛЕКТОРА
-- ============================================================
-- 1.00 = обычный размер
-- 1.20 = на 20% шире/выше
-- 0.80 = на 20% уже/ниже
--
-- Эти значения растягивают только бумажную картинку/геометрию круга.
-- Текст и иконки остаются на обычных позициях.
local FC_Q_SCALE_X = 1.80 -- ширина
local FC_Q_SCALE_Y = 1.20 -- высота

-- Общий радиус меню относительно высоты экрана.
-- Чем меньше число, тем меньше весь Q-селектор.
local FC_Q_RADIUS_NORMAL = 0.34
local FC_Q_RADIUS_SUBMENU = 0.30

-- Увеличение выбранного сектора.
-- Поставь 0, чтобы сектор не выпирал при наведении.
local FC_Q_HOVER_GROW = 0.00


-- Normal and hovered tint. White preserves the original PNG colors.
local FC_Q_NORMAL_COLOR = Color(255, 255, 255, 235)
local FC_Q_HOVER_COLOR = Color(170, 25, 20, 115)

-- Сила красного слоя при наведении.
-- Альфа 0 = невидимый слой, 255 = почти полностью красный.
local FC_Q_HOVER_ALPHA = 115
...
local function CreateRadialMenu(options_arg, bAutoClose)
	...
	menuPanel.Paint = function(self, w, h)
		...
		for num, option in ipairs(options) do
			...
			local r = scrH * (options_arg ~= nil and FC_Q_RADIUS_SUBMENU or FC_Q_RADIUS_NORMAL) * viewLerp
			local partDeg = 360 / #options
			local hitX = x / math.max(FC_Q_SCALE_X, 0.001)
			local hitY = y / math.max(FC_Q_SCALE_Y, 0.001)
			local sqrt = math.sqrt(hitX ^ 2 + hitY ^ 2)
			...
			if option[3] then --// Multibutton
				-- Базовая бумажная картинка всегда остаётся видимой.
				surface.SetMaterial(FC_Q_SECTOR)
				surface.SetDrawColor(FC_Q_NORMAL_COLOR)
				draw.CirclePart(
					w / 2,
					h / 2,
					r,
				...
				-- При наведении поверх рисуется увеличенная красная копия.
				...
				for i, opt in pairs(option[4]) do
					...
					draw.CirclePart(w / 2, h / 2, r * (i / count), 40, #options, num, FC_Q_SCALE_X, FC_Q_SCALE_Y)
					...
					draw.DrawText(opt, "FC_Q_Newspaper", scrW / 2 + math.sin(a) * r * (i / count - 0.5 / count), scrH / 2 + math.cos(a) * r * (i / count - 0.5 / count), colWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end

				continue
			end
			
			-- Основной бумажный сектор всегда рисуется полностью.
			surface.SetMaterial(FC_Q_SECTOR)

			local normalSectorColor = FC_Q_NORMAL_COLOR
			if option[6] and IsColor(option[6]) then
				normalSectorColor = option[6]
			end

			surface.SetDrawColor(normalSectorColor)
			draw.CirclePart(
				w / 2,
				h / 2,
				r,
			...
			-- Наведённый сектор: увеличенная полупрозрачная красная копия.
			-- Благодаря двум слоям исходная PNG не исчезает.
			if isMouseIntersecting then
				local hoverColor = FC_Q_HOVER_COLOR
				...
				draw.CirclePart(
					w / 2,
					h / 2,
					r * (1 + FC_Q_HOVER_GROW * optionSelected[num]),
					40,
					#options,
					num,
					FC_Q_SCALE_X,
			...
			if option[5] then --// Icon
				...
				draw.DrawText(txt, "FC_Q_Newspaper", scrW / 2 + math.sin(a) * r * 0.75, scrH / 2 + math.cos(a) * r * 0.75, colWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
		if !paining then
