...
local teams = {
	[0] = {
		objective = "Убейте Всех Солдат Альянса.",
		name = "Постановец",
	...
	[1] = {
        objective = "Ликвидируйте Постановцев",
        name = "Солдат Альянса",
        name_elite = "Вы Элитный Солдат Альянса",
        name_shotgunner = "Солдат Альянса с дробовиком",
...
if zb.ROUND_START + 8.5 < CurTime() then return end
    ...
    local role = IsValid(lply) and lply:GetNWString("PlayerRole") or ""
    local team_data = teams[team_id] or teams[0]

    draw.SimpleText("FreakBattle | Half-Life 2 Deathmatch", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

	--; Любимое ООП шарика
    local role_data = {
        name = team_data.name,
        color = Color(team_data.color1.r, team_data.color1.g, team_data.color1.b),
    ...
    local objective_color = Color(team_data.color2.r, team_data.color2.g, team_data.color2.b)
...
hook.Add("radialOptions", "CMB_Airstrike", function()
    ...
    if IsValid(lply) and lply:GetNWString("PlayerRole") == "Elite" and org and not org.otrub then -- that's a feature apparently
		local tbl = {
			function()
...
CreateEndMenu = function()
	...
	for i,ply in player.Iterator() do
		...
		function but:DoClick()
			if not IsValid(ply) or ply:IsBot() then chat.AddText(Color(255,0,0), "no, you can't") return end
