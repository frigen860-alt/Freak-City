...
local function ZC_RoleBG_Draw(startTime, showTime)
    ...
    draw.RoundedBox(0, 0, yoff, w, h, Color(36, 20, 8, 232 * alphaMul))

    surface.SetMaterial(ZC_RoleBG_GradientD)
    surface.SetDrawColor(255, 120, 35, 22 * alphaMul)
    ...
    surface.SetDrawColor(255, 145, 45, 10 * alphaMul)
    ...
    surface.SetDrawColor(255, 160, 60, 18 * alphaMul)
    ...
    surface.SetDrawColor(255, 145, 45, 55 * alphaMul)
...
net.Receive("homerhunt_start",function()
	roundend = false
	...
	HomerHunt_Homer = net.ReadEntity()
end)

local fighter = {
	objective_homer = "Ты Гомер. У тебя силы Superfighters. Переживи охоту и уничтожь Бартов.",
	objective_bart = "Ты Барт. Найди Гомера и убей его вместе с другими Бартами.",
	name_homer = "Гомер",
	name_bart = "Барт",
	color_homer = Color(255, 190, 45),
	color_bart = Color(255, 220, 65)
}
...
if zb.ROUND_START + 5 > CurTime() then
		...
		if IsValid(dmmusic) then
			...
			local homerMusicVolume = 0.35 -- громкость музыки режима: 0.35 = 35% от старой
			dmmusic:SetVolume(vol * musicVolume * homerMusicVolume)
		end
	end
    ...
    draw.SimpleText("Охота на Гомера | FreakBattle", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(255,190,45, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

	local isHomer = LocalPlayer():GetNWBool("HomerHuntHomer", false)

    local Rolename = isHomer and fighter.name_homer or fighter.name_bart
	local ColorRole = isHomer and fighter.color_homer or fighter.color_bart
    ...
    local Objective = isHomer and fighter.objective_homer or fighter.objective_bart
    local ColorObj = ColorRole
    ColorObj.a = 255 * fade
...
net.Receive("homerhunt_end",function()
	local winner = net.ReadString()
	local ent = net.ReadEntity()

	wonply = nil
	HomerHuntWinner = winner

	if IsValid(ent) then
		ent.won = winner == "homer"
		wonply = ent
	end
...
local colRed = Color(255,190,45)
local colRedUp = Color(255,220,80)
...
CreateEndMenu = function()
    ...
    hmcdEndMenu.Paint = function(self,w,h)
		BlurBackground(self)
		local txt = (HomerHuntWinner == "barts" and "Барты победили! Гомер убит!" or "Гомер победил!")
