...
local hg_font = ConVarExists("hg_font") and GetConVar("hg_font") or CreateClientConVar("hg_font", "Special Elite (Rus by Lomzz)", true, false, "Change UI text font")

local font = function()
    return "Special Elite (Rus by Lomzz)"
end
...
-- ============================================================
-- FreakCity custom TAB / Scoreboard
--
-- Фон TAB:
--   materials/freakcity/tab/background.png
--
-- Фон карточки игрока:
--   materials/freakcity/tab/playercard.png
--
-- Обе картинки автоматически растягиваются под размер панели.
-- Если файла нет, используется тёмный запасной фон.
-- ============================================================

local ZB_TAB_COLORS = {
	bg = Color(8, 8, 10, 235),
	panel = Color(10, 10, 12, 120),
	panelDark = Color(5, 5, 7, 145),
	border = Color(215, 205, 185, 90),
	borderHover = Color(235, 225, 205, 155),
	button = Color(18, 18, 21, 220),
	buttonActive = Color(52, 48, 45, 235),
	buttonHover = Color(40, 37, 35, 235),
	white = Color(245, 242, 235, 255),
	muted = Color(175, 170, 162, 220),
	ping = Color(225, 220, 208, 255),
}

-- Пути можно менять здесь.
local ZB_TAB_BACKGROUND_FILE = "materials/freakcity/tab/background.png"
local ZB_TAB_BACKGROUND_PATH = "freakcity/tab/background.png"

local ZB_TAB_PLAYERCARD_FILE = "materials/freakcity/tab/playercard.png"
local ZB_TAB_PLAYERCARD_PATH = "freakcity/tab/playercard.png"

-- Прозрачность затемнения поверх фоновой картинки.
-- 0 = без затемнения, 255 = полностью чёрный фон.
local ZB_TAB_BACKGROUND_SHADE = 35

-- Затемнение поверх карточки игрока.
local ZB_TAB_CARD_SHADE = 15

local ZB_TAB_BACKGROUND_MATERIAL = Material(ZB_TAB_BACKGROUND_PATH, "smooth noclamp")
local ZB_TAB_PLAYERCARD_MATERIAL = Material(ZB_TAB_PLAYERCARD_PATH, "smooth noclamp")


-- ===============================
-- НАСТРОЙКА РАЗМЕРА PLAYERCARD
-- ===============================

local PLAYER_CARD_SCALE_X = 1.0 -- ширина
local PLAYER_CARD_SCALE_Y = 1.5 -- высота

local PLAYER_CARD_OFFSET_X = 0
local PLAYER_CARD_OFFSET_Y = 0

local function ZB_IsUsableMaterial(filePath, material)
	return file.Exists(filePath, "GAME")
		and material
		and not material:IsError()
end

local function ZB_DrawTabBackground(w, h)
	if ZB_IsUsableMaterial(ZB_TAB_BACKGROUND_FILE, ZB_TAB_BACKGROUND_MATERIAL) then
		surface.SetMaterial(ZB_TAB_BACKGROUND_MATERIAL)
		surface.SetDrawColor(255, 255, 255, 255)
		surface.DrawTexturedRect(0, 0, w, h)
	else
		surface.SetDrawColor(ZB_TAB_COLORS.bg)
		surface.DrawRect(0, 0, w, h)
	end

	if ZB_TAB_BACKGROUND_SHADE > 0 then
		surface.SetDrawColor(0, 0, 0, ZB_TAB_BACKGROUND_SHADE)
		surface.DrawRect(0, 0, w, h)
	end

	surface.SetDrawColor(ZB_TAB_COLORS.border)
	surface.DrawOutlinedRect(0, 0, w, h, 1)
end

local function ZB_DrawPlayerCard(self, w, h, spectator)
	local hovered = self:IsHovered()
	local cardExists = ZB_IsUsableMaterial(ZB_TAB_PLAYERCARD_FILE, ZB_TAB_PLAYERCARD_MATERIAL)

	if cardExists then
		surface.SetMaterial(ZB_TAB_PLAYERCARD_MATERIAL)
		surface.SetDrawColor(255, 255, 255, hovered and 255 or 235)
		local cardW = w * PLAYER_CARD_SCALE_X
        local cardH = h * PLAYER_CARD_SCALE_Y

         surface.DrawTexturedRect(
          PLAYER_CARD_OFFSET_X,
          PLAYER_CARD_OFFSET_Y,
          cardW,
          cardH
)

		if ZB_TAB_CARD_SHADE > 0 then
			surface.SetDrawColor(0, 0, 0, ZB_TAB_CARD_SHADE)
			surface.DrawRect(0, 0, w, h)
		end
	else
		local fallback = spectator and Color(20, 21, 25, 225) or Color(19, 20, 23, 230)
		if hovered then
			fallback = Color(39, 36, 34, 235)
		end

		draw.RoundedBox(3, 0, 0, w, h, fallback)
	end

	if hovered then
		surface.SetDrawColor(255, 245, 225, 18)
		surface.DrawRect(1, 1, w - 2, h - 2)
	end

	surface.SetDrawColor(hovered and ZB_TAB_COLORS.borderHover or ZB_TAB_COLORS.border)
	surface.DrawOutlinedRect(0, 0, w, h, 1)
end

local function ZB_GetULXRankText(ply)
	...
	return names[group] or string.upper(string.sub(group, 1, 1)) .. string.sub(group, 2)
end

local function ZB_GetULXRankColor(ply)
	if not IsValid(ply) then return Color(150, 150, 150) end

	local group = ply.GetUserGroup
		and string.lower(tostring(ply:GetUserGroup() or "user"))
		or "user"

	-- Оранжевые цвета удалены.
	local colors = {
		superadmin = Color(210, 70, 70),
		admin = Color(190, 85, 85),
		moderator = Color(100, 170, 225),
		operator = Color(120, 165, 215),
		vip = Color(120, 205, 125),
		user = Color(175, 175, 175),
		bot = Color(145, 145, 145),
	}

	return colors[group] or Color(190, 190, 190)
end

-- Название оставлено старым, чтобы не менять вызовы по всему файлу.
-- Визуально кнопка больше не оранжевая.
local function ZB_PaintSmallOrangeButton(self, w, h, text, active)
	local hovered = self:IsHovered()

	local bg
	if active then
		bg = ZB_TAB_COLORS.buttonActive
	elseif hovered then
		bg = ZB_TAB_COLORS.buttonHover
	else
		bg = ZB_TAB_COLORS.button
	end
...
local function ZB_PaintPlayerRow(self, w, h, ply, spectator)
	if not IsValid(ply) then return end

	ZB_DrawPlayerCard(self, w, h, spectator)

	local name = ply:Name() or "Disconnected player"
	local rankText = ZB_GetULXRankText(ply)
	local rankColor = ZB_GetULXRankColor(ply)
	local pingText = tostring(ply:Ping() or 0)

	-- Оставляем место слева под AvatarImage.
	local textX = math.max(ScreenScaleH(30), h + 5)

	-- Справа оставляем место под кнопку звука.
	local pingX = w - ScreenScaleH(39)
...
local function ZB_AddPlayerAvatar(parent, ply)
	if not IsValid(parent) or not IsValid(ply) then return end

	local avatar = vgui.Create("AvatarImage", parent)
	avatar:Dock(LEFT)
	avatar:SetWide(ScreenScaleH(24))
	avatar:DockMargin(ScreenScaleH(3), ScreenScaleH(3), ScreenScaleH(5), ScreenScaleH(3))
	avatar:SetMouseInputEnabled(false)
	avatar:SetPlayer(ply, 64)

	parent.PlayerAvatar = avatar
end

local function ZB_StyleScrollPanel(panel)
	if not IsValid(panel) then return end

	panel.Paint = function(_, w, h)
		surface.SetDrawColor(ZB_TAB_COLORS.panel)
		surface.DrawRect(0, 0, w, h)
	...
	bar.btnGrip.Paint = function(_, w, h)
		surface.SetDrawColor(115, 110, 105, 180)
...
function GM:ScoreboardShow()
	...
	-- Убираем старые цвета ZFrame.
	scoreBoardMenu:SetColorBG(Color(0, 0, 0, 0))
	scoreBoardMenu:SetColorBR(Color(0, 0, 0, 0))

	if scoreBoardMenu.SetBlurStrengh then
		scoreBoardMenu:SetBlurStrengh(1.2)
	end

	local bottomButtonW = ScreenScale(30)
	local bottomButtonH = ScreenScale(6)

	local muteallbut = vgui.Create("DButton", scoreBoardMenu)
	muteallbut:SetPos(
		scoreBoardMenu:GetWide() - bottomButtonW * 2.3,
		scoreBoardMenu:GetTall() - bottomButtonH * 1.5
	)
	muteallbut:SetSize(bottomButtonW, bottomButtonH)
	muteallbut:SetText("")

	muteallbut.Paint = function(self, w, h)
		ZB_PaintSmallOrangeButton(self, w, h, "Mute all", hg.muteall)
	end
	...
	muteallbut.DoClick = function()
		...
		for _, ply in player.Iterator() do
			if hg.muteall then
				...
				local savedVolume = hg.playerInfo[ply:SteamID()]
					and hg.playerInfo[ply:SteamID()][2]
					or 1

				ply:SetVoiceVolumeScale(
					(not hg.mutespect or ply:Alive())
					and savedVolume
					or 0
				)
			end
	...
	mutespectbut:SetPos(
		scoreBoardMenu:GetWide() - bottomButtonW * 1.2,
		scoreBoardMenu:GetTall() - bottomButtonH * 1.5
	)
	mutespectbut:SetSize(bottomButtonW, bottomButtonH)
	mutespectbut:SetText("")

	mutespectbut.Paint = function(self, w, h)
		ZB_PaintSmallOrangeButton(self, w, h, "Mute spect.", hg.mutespect)
	end
	...
	mutespectbut.DoClick = function()
		...
		for _, ply in player.Iterator() do
			...
			if hg.mutespect then
				...
				ply:SetVoiceVolumeScale(not hg.muteall and savedVolume or 0)
			end
		end
	end
	...
	scoreBoardMenu.Paint = function(self, w, h)
		if hg and hg.DrawBlur then
			hg.DrawBlur(self, 1.2, nil, 80)
		end

		ZB_DrawTabBackground(w, h)
	end

	scoreBoardMenu.PaintOver = function(self, w, h)
		surface.SetFont("ZB_InterfaceLarge")
		surface.SetTextColor(ZB_TAB_COLORS.white)

		local textW, textH = surface.GetTextSize(serverName)
		surface.SetTextPos(w / 2 - textW / 2, 10)
		surface.DrawText(serverName)

		surface.SetFont("ZB_InterfaceSmall")
		surface.SetTextColor(ZB_TAB_COLORS.muted)

		local versionText = "ZC Version: " .. tostring(hg.Version or "unknown")
		textW, textH = surface.GetTextSize(versionText)
		surface.SetTextPos(w * 0.01, h - textH - h * 0.01)
		surface.DrawText(versionText)

		surface.SetFont("ZB_InterfaceMediumLarge")
		surface.SetTextColor(ZB_TAB_COLORS.white)

		textW = surface.GetTextSize("Игроки:")
		surface.SetTextPos(w / 4 - textW / 2, ScreenScale(25))
		surface.DrawText("Игроки:")

		textW = surface.GetTextSize("Наблюдатели:")
		surface.SetTextPos(w * 0.75 - textW / 2, ScreenScale(25))
		surface.DrawText("Наблюдатели:")

		tick = math.Round(1 / engine.ServerFrameTime())

		local tickText = "SV Tick: " .. tick
		textW = surface.GetTextSize(tickText)
		surface.SetTextPos(w * 0.5 - textW / 2, ScreenScale(25))
		surface.DrawText(tickText)
	end

	local function CreateJoinButton(x, y, joinSpectators)
		local button = vgui.Create("DButton", scoreBoardMenu)
		button:SetPos(x, y)
		button:SetSize(ScrW() / 20, ScrH() / 30)
		button:SetText("")

		button.Paint = function(self, w, h)
			ZB_PaintSmallOrangeButton(self, w, h, "Join", false)
		end

		button.DoClick = function()
			net.Start("ZB_SpecMode")
				net.WriteBool(joinSpectators)
			net.SendToServer()
	...
	if LocalPlayer():Team() ~= TEAM_SPECTATOR then
		CreateJoinButton(sizeX * 0.925, sizeY * 0.095, true)
	else
		CreateJoinButton(sizeX * 0.010, sizeY * 0.095, false)
	end
	...
	ZB_StyleScrollPanel(playersPanel)
	...
	for _, ply in player.Iterator() do
		...
		if curRound and curRound.name == "fear" and not ply:Alive() then continue end
		if disappearance and ply ~= lply then continue end

		local row = vgui.Create("DButton", playersPanel)
		row:SetSize(100, ScreenScaleH(30))
		row:Dock(TOP)
		row:DockMargin(8, 6, 8, -1)
		row:SetText("")

		ZB_AddPlayerAvatar(row, ply)

		local soundButton = vgui.Create("DImageButton", row)
		soundButton:Dock(RIGHT)
		soundButton:SetWide(ScreenScaleH(22))
		soundButton:DockMargin(5, ScreenScaleH(7), ScreenScaleH(6), ScreenScaleH(7))
		...
		row.Paint = function(self, w, h)
			ZB_PaintPlayerRow(self, w, h, ply, false)
		end

		row.DoClick = function()
			if ply:IsBot() then
				chat.AddText(Color(210, 75, 75), "Bots do not have a Steam profile.")
				return
			end
	...
	ZB_StyleScrollPanel(spectatorsPanel)
	...
	for _, ply in player.Iterator() do
		...
		local row = vgui.Create("DButton", spectatorsPanel)
		row:SetSize(100, ScreenScaleH(30))
		row:Dock(TOP)
		row:DockMargin(8, 6, 8, -1)
		row:SetText("")
		...
		row.Paint = function(self, w, h)
			ZB_PaintPlayerRow(self, w, h, ply, true)
		end

		row.DoClick = function()
			if ply:IsBot() then
				chat.AddText(Color(180, 180, 180), "Bots do not have a Steam profile.")
				return
			end
